...
import { deleteTweet } from "~/store/tweet";
      ...
      {isLoading ? (
        ...
        <>
          <div className="w-full max-h-screen">
            {posts1.map((post1) => (
              <div key={post1.id} className="pt-3 w-full min-h-[100px] flex bg-black transition-all hover:bg-[#3c3c3c0a] cursor-pointer">
                ...
                <div className="w-[518.4px] flex flex-col mr-5">
                  <div className="w-full h-[20px] flex">
                    <div className="w-[302.1px] flex h-full items-center">
                      ...
                      <p className="hover:underline text-[#64686d]"><span className="text-[#64686d] px-1">·</span>{formatDate(post1.createdAt)}</p>
                    </div>

                    <div className="w-[18.75px] h-full ml-[190px] -mt-1.5 relative">
                      <div
                        className="w-[34.75px] h-[34.75px] group transition-colors duration-100 rounded-full flex items-center justify-center hover:bg-[#1c9aee17]"
                        onClick={() => setShowDeletePopover(showDeletePopover === post1.id ? null : post1.id)}
                      >
                      ...
                      {showDeletePopover === post1.id && (
                        <div
                          ref={popoverRef}
                          className="absolute right-0 mt-2 w-64 bg-black border border-[#2f3336] rounded-xl shadow-lg z-50"
                        >
                          <div className="p-4">
                            <h3 className="text-white font-bold mb-2">Postu Sil</h3>
                            <p className="text-[#71767b] text-sm mb-4">Bu postu silmek istediğinizden emin misiniz? Bu işlem geri alınamaz.</p>
                            <button
                              onClick={() => handleDelete(post1.id)}
                              className="w-full py-2 px-4 bg-red-500 text-white rounded-full font-bold hover:bg-red-600 transition-colors"
                            >
                              Sil
                            </button>
                          </div>
                  ...
                  <div className="w-full py-2">
                    <p className="text-[#e7e9ea]">{post1.content}</p>
                  </div>
          ...
          <div>
            {forMainHomePosts.map((post) => (
              <div
                key={post.id}
                className="bg-black transition-all border-t-[1px] border-[#2f3336] hover:bg-[#3c3c3c0a] cursor-pointer min-h-[100px]"
              >
                <div className="w-full h-[12px]"></div>
                <div className="w-[566.4px] h-full ml-[16.8px] flex">
                  ...
                  <div className="w-[518.4px] h-full flex flex-col pl-2">
                    ...
                    <div className="w-full py-2">
                      <p className="text-[#e7e9ea]">{post.content}</p>
                    </div>
